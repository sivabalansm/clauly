...
export default function DevHarness() {
  ...
  const [customOriginal, setCustomOriginal] = useState<string>(
    "This is the first clasue redline it"
  );
  const [customProposed, setCustomProposed] = useState<string>(
    "This is the first clause to redline."
  );
  ...
  const setupContract = wrap("Load sample contract", async () => {
    return await Word.run(async (context) => {
      ...
      await context.sync();
      return {
        paragraphs_inserted: SAMPLE_CONTRACT.length,
        rejected_existing: existing.items.length,
      };
    });
  });
  ...
  const clearAll = wrap("Clear document", async () => {
    return await Word.run(async (context) => {
      ...
      context.document.body.clear();
      await context.sync();
      return { cleared: true, rejected_existing: existing.items.length };
    });
  });
  ...
  const runApplyCustom = wrap("applyRedline (custom)", () =>
    applyRedline({
      id: "custom_" + Date.now(),
      clause_id: "custom",
      clause_type: "other",
      original_text: customOriginal,
      proposed_text: customProposed,
      reasoning: "Custom test from the dev harness",
      severity: "market",
      market_evidence: [],
    })
  );
  ...
  return (
    ...
    >
      ...
      <div style={sectionStyle}>
        <div style={headerStyle}>Custom redline (test on your real doc)</div>
        <div style={{ fontSize: "10px", color: "#6e6e73", marginBottom: "4px" }}>
          Paste an exact line from your Word doc into "Original", then write what
          you want it replaced with into "Proposed". Click apply.
        </div>
        <label style={{ fontSize: "10px", color: "#3a3a3c", fontWeight: 600 }}>
          Original (must match doc exactly)
        </label>
        <textarea
        ...
        />
        <label style={{ fontSize: "10px", color: "#3a3a3c", fontWeight: 600 }}>
          Proposed (what to redline it to)
        </label>
        <textarea
        ...
        />
        <button
          style={{ ...buttonStyle, background: "#1d8348", color: "#fff" }}
          onClick={runApplyCustom}
          disabled={running || !customOriginal.trim() || !customProposed.trim()}
        >
          Apply custom redline
        </button>
      </div>
